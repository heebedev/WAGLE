...
<%
    ...
    String A = "select A.* from(select wc.*, count(*) from WagleUser as wu, WagleCreate as wc where wu.wcSeqno = wc.wcSeqno and Moim_mSeqno = '" + moimSeq+ "' and wuValidation = 1 and wcDueDate >= date(now()) group by wu.wcSeqno order by count(*) desc) as A union all ";
    String B = "select B.* from(select wc.*, count(*) from WagleUser as wu, WagleCreate as wc where wu.wcSeqno = wc.wcSeqno and Moim_mSeqno = '" + moimSeq+ "' and wuValidation = 1 and wcDueDate < date(now()) group by wu.wcSeqno order by count(*) desc) as B;";
